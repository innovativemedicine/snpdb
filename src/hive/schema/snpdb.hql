CREATE TABLE variant (
    ...
    chromosome string, -- maxlen(Chr) == 21, 0 nulls
    -- fields 23,24,29,30,35,36,41 (23 == Start, 24 == End) were all the same
    start_posn int, -- >= 1 ? 0 nulls
    end_posn int, -- >= 1 ? 0 nulls
    -- fields 26,32,38 (26 == Obs, 32,38 == unlabeled) were all the same
    -- alt string, -- maxlen(Obs) == 1, 0 nulls, I think (([ACGTN]+|<$ID>);)+, arbitrary choice
    -- fields 25,31,37,43 (25 == Ref, 31,37,43 == unlabeled) were all the same
    -- the reference allele
    ref string, -- maxlen(Ref) == 1, 0 nulls, I think [ACGTN]+, arbitrary choice
    alt_alleles array<string>, 
    dbsnp_id string, -- field #42

    -- genotype_format string, -- field #48
    quality double, -- field #45, "traditionally people use int phred scores, this field is permitted to be a floating point to enable higher resolution for low confidence calls if desired" (not defined in summary file for whatever reason)
    filter string, -- field #46, maxlen() == 27 "traditionally people use int phred scores, this field is permitted to be a floating point to enable higher resolution for low confidence calls if desired" (not defined in summary file for whatever reason)
    ...
    -- strings
    func string, -- maxlen(Func) == 19, 0 nulls
    gene string, -- maxlen(Gene) == 184, 0 nulls
    exonicfunc string, -- maxlen(ExonicFunc) == 17, 66012 nulls
    aachange string, -- maxlen(AAChange) == 31, 66012 nulls
    conserved string, -- maxlen(Conserved) == 18, 61461 nulls
    1000g2011may_all string, -- 70732 nulls
    dbsnp135 string, -- 70732 nulls
    ljb_phylop_pred string, -- maxlen(LJB_PhyloP_Pred) == 1, 68762 nulls
    ljb_sift_pred string, -- maxlen(LJB_SIFT_Pred) == 2, 68762 nulls
    ljb_polyphen2_pred string, -- maxlen(LJB_PolyPhen2_Pred) == 2, 68762 nulls
    ljb_lrt_pred string, -- maxlen(LJB_LRT_Pred) == 2, 68762 nulls
    ljb_mutationtaster_pred string, -- maxlen(LJB_MutationTaster_Pred) == 2, 68762 nulls
    -- columns 27,33 (27 == Otherinfo, 33 unlabeled) were all the same
    otherinfo string, -- maxlen(Otherinfo) == 7, 0 nulls
    ...
    vc map<bigint, 
        struct<
            ...
            -- just a duplicate of the key that indexed it...may want this for convenience
            patient_id: bigint, -- not null,

            -- maybe useful to retain fields that aren't defined in this table... but in that case one 
            -- should consider adding a new vc_group_info_* table, or add desired fields to an existing 
            -- vc_group_info_* table (perhaps this should be of TEXT type?)
            -- genotype_source string, -- field #49-60
